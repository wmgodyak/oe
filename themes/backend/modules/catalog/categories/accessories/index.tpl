<fieldset>
    <legend>{$t.catalog.accessories.index}</legend>

    <div class="form-group">
        <label for="acc_categories" class="col-sm-3 control-label">{$t.catalog.accessories.categories_select}</label>
        <div class="col-sm-9">
            <select id="acc_categories" class="form-control" data-placeholder="{$t.catalog.accessories.categories_select_p}"></select>
        </div>
    </div>
    <div id="acc_categories_list"></div>
</fieldset>
<script>var t_sa = {json_encode($t.catalog.accessories)}</script>
{literal}
<script type="text/template" id="acc_categories_tpl">
    ...
    <table class="table-bordered table">
        ...
        <tr>
            ...
            <td>
                <a href="module/run/catalog/categories/edit/<%- item.categories_id %>" title="Редагувати" target="_blank"><%- item.name %></a>
