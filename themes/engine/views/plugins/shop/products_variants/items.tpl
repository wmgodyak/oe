        ...
        <tr id="variant-{$variant.id}">
            ...
            <td>
                <select name="variants[{$variant.id}][in_stock]" class="form-control">
                    <option {if $variant.in_stock == 1}selected{/if} value="1">В наявності</option>
                    <option {if $variant.in_stock == 2}selected{/if}  value="2">Під замовлення</option>
                    <option {if $variant.in_stock == 0}selected{/if}  value="0">Немає</option>
                </select>
            </td>
...
{*<pre>{print_r($customers_group)}</pre>*}
