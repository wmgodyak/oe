...
engine.payment = {
    init: function()
    {
        ...
        $(document).on('change', '#data_module', function() {
            ...
            if(module == '') return;
            engine.request.post({
                url: 'module/run/payment/getModuleSettings',
                data: {module: module},
    ...
    create: function()
    {
        engine.request.get('module/run/payment/create', function(d)
        {
            var buttons = [
                {
    ...
    {
        engine.request.post({
            url: 'module/run/payment/edit/' + id,
            data: {id: id},
            success: function(d)
            {
                var buttons = [
    ...
    delete: function(id)
    {
        ...
        (
            ...
            function()
            {
                engine.request.get('module/run/payment/delete/' + id, function(d){
                    if(d > 0){
                        engine.refreshDataTable('payment');
                    }
                });
    ...
    pub: function(id)
    {
        engine.request.get('module/run/payment/pub/' + id, function(d){
            if(d > 0){
                engine.refreshDataTable('payment');
            }
        });
    ...
    hide: function(id)
    {
        engine.request.get('module/run/payment/hide/' + id, function(d){
            if(d > 0){
                engine.refreshDataTable('payment');
            }
        });
