...
namespace modules\catalog\models\backend\products;

use modules\catalog\models\backend\Prices;
use modules\catalog\models\backend\products\Features;
use modules\catalog\models\backend\ProductsFeatures;
use modules\catalog\models\backend\ProductsPrices;
use system\models\UsersGroup;

class Variants extends \modules\catalog\models\products\variants\Variants
{
    private $pv;
    private $pvf;
    private $features;
    ...
    public function __construct()
    {
        ...
        $this->pv = new variants\ProductsVariants();
        $this->pvf = new variants\ProductsVariantsFeatures();
        $this->features = new ProductsFeatures();
        $this->pp = new ProductsPrices();
        $this->ug = new UsersGroup();
        $this->pvp = new variants\ProductsVariantsPrices();
    }
...
//        $this->updateRow('__content', $content_id, ['has_variants' => 1]);
    ...
    {
        $r= self::$db->select("select id, in_stock,sku, img from __products_variants where content_id = '{$content_id}' ")->all();
        $res = [];
        foreach ($r as $row) {
    ...
    public function update($content_id)
    {
        ...
        foreach ($variants as $variant_id=>$a) {
            $this->updateRow('__products_variants', $variant_id, ['in_stock' => $a['in_stock'], 'sku' => $a['sku']]);
