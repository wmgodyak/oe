        ...
        <div class="sort-select-group">
            <select class="jq-select" name="sort" onchange="submit()">
                {foreach $sorting.items as $item}
                    <option {if !isset($smarty.get.sort) && $item.external_id=='popular'}selected{/if} {if isset($smarty.get.sort) && $smarty.get.sort == $item.external_id}selected{/if} value="{$item.external_id}">{$item.name}</option>
                {/foreach}
            </select>
        </div>
