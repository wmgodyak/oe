...
class Accessories extends Model
{
    ...
    private function getCategories($categories_id)
    {
        ...
        $categories = self::$db
            ->select("
                ...
                where pc.products_categories_id = '{$categories_id}'
                ")
            ->all();
        ...
        if(empty($categories)){
            $main_cat_id = self::$db
                ...
                ->row('parent_id');
            if($main_cat_id > 0){
                return $this->getCategories($main_cat_id);
            }
        }
    ...
    public function get($products_id)
    {
       ...
       $products_categories_id = $this->relations->getMainCategoriesId($products_id);
        ...
        if(empty($products_categories_id)) return null;

        $categories = $this->getCategories($products_categories_id);
