...
class ImagesSizes extends Backend
{
    ...
    public function update($id)
    {
        ...
        if (!empty($selected)) {
            self::$db
                ->delete(
                    '__content_types_images_sizes',
                    " images_sizes_id={$id} and types_id in (". implode(',', $selected) .")"
                );
        }
    ...
    public function getSelectedTypes($images_sizes_id)
    {
        $r = [];
        $types = self::$db
            ->select("select types_id from __content_types_images_sizes where images_sizes_id={$images_sizes_id}")
            ->all();
    ...
    public function resizeItems($sizes_id, $start, $num = 1)
    {
        ...
        $watermark_src = $this->config->images_watermark_dir;
        $source = $this->config->images_source_dir;
        ...
        foreach ($r as $item) {
            ...
            if (!is_dir(DOCROOT. $size_path))
                mkdir(DOCROOT . $size_path, 0775, true);
