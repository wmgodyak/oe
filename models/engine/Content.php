...
class Content extends Engine
{
    ...
    {
        $s = parent::createRow('content', $data);
        if($s>0 && $data['parent_id'] > 0){
            self::$db->update('content', ['isfolder'=>1], "id={$data['parent_id']} limit 1");
        }
        return $s;
    }
    ...
    {
        return parent::updateRow('content', $id, $data);
    }
    ...
    public function delete($id)
    {
        ...
        $s = parent::deleteRow('content', $id); // TODO: Change the autogenerated stub

        if($s > 0 && $data['parent_id'] > 0){
            if(! $this->hasChildren($data['parent_id'])){
                parent::updateRow('content', $data['parent_id'], ['isfolder' => 0]);
            }
        }
    ...
    {
        return self::$db
            ->select("select count(id) as t from content where parent_id={$id}")
            ->row('t') > 0;
    }
