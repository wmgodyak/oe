CKEDITOR.editorConfig = function( config ) {
	...
	config.extraPlugins = 'codemirror,inlinesave';
	config.inlinesave = {
		postUrl: '/route/adminPanel/updateContent',
		postData: {token: TOKEN, id: CONTENT_ID},
		...
		onSuccess: function(editor, data) {
			var elem =$('#'+editor.name);
			elem.blur();
			elem.notify("Update success", "success");
		}
	};

	config.toolbar = [
		{ name: 'document', items : [ 'Inlinesave', 'Paste','PasteText','PasteFromWord','-','Undo','Redo'  ] },
